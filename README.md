...
- **戸籍書類の解析** — PDF・画像（JPEG/PNG/WebP）を複数まとめてアップロード → 最新のAIで自動解析 → 家系図へ取り込み。取り込みの入り口はひとつで、選択・ドラッグ＆ドロップ・**貼り付け（Ctrl/⌘+V）**のどれでも追加できます。**続けて並んだ画像はまとめて通しで読み取り**（スマホで数枚に分けて撮った場合）、PDFは1件ずつ読み取ります。取り込むものはすべて1つの家系図のための書類のため、戸籍ごとに分けて取り込む必要はありません。**Gemini 3.1 Pro / Claude Opus 5 / GPT-5.2 のマルチプロバイダ構成**で、障害・モデル未提供時は自動フォールバック（選定根拠は [docs/MODEL_RESEARCH.md](./docs/MODEL_RESEARCH.md)）。ファイルは案件に紐づけて非公開ストレージに保存され、再解析・ダウンロード・削除ができます
...
- **読み取り前の画像処理** — 撮影画像の向きを正規化し、大きすぎるものを縮小してからAIへ渡します。グレースケール化・コントラスト伸長は切り替えで用意し、実データでの効果を測ってから既定にするかを決めます（`pnpm benchmark -- --preprocess`）
