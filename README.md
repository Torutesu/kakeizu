...
- **戸籍書類の解析** — PDF・画像（JPEG/PNG/WebP）を複数まとめてアップロード → 最新のAIで自動解析 → 家系図へ取り込み。**1通の戸籍が複数枚に分かれている場合（スマホ撮影など）は、まとめて1通として通しで読み取れます**。**Gemini 3.1 Pro / Claude Opus 5 / GPT-5.2 のマルチプロバイダ構成**で、障害・モデル未提供時は自動フォールバック（選定根拠は [docs/MODEL_RESEARCH.md](./docs/MODEL_RESEARCH.md)）。ファイルは案件に紐づけて非公開ストレージに保存され、再解析・ダウンロード・削除ができます
