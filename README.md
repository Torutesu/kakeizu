...
- **招待制** — 未招待のアドレスは登録自体をDB層で拒否。招待メールのリンクからパスワードを設定するとすぐに参加できる。パスワードの再設定・変更も画面から行える
...
pnpm verify:db   # マイグレーションの適用とアクセス制御（RLS）の検証（ローカルにPostgresが必要）
pnpm build       # プロダクションビルド
pnpm e2e         # ブラウザでの実挙動 (Playwright)
```
...
- `lib/db/` — データアクセス層（案件・メンバー・家系図・戸籍ファイル）
- `lib/auth/authLinks.ts` / `app/auth/` — 招待・パスワード再設定メールのリンクを受けてセッションを確立する処理
...
- `public/` 配下はビルドに含まれ、URLを知っていれば誰でも取得できます。実データを置かないでください（テストデータは gitignore 済みの `testdata/` へ）
