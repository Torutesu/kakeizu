...
// ============================================================================
// システム指示（役割・進め方・ルール）
// 全プロバイダ（Gemini / Claude / GPT）共通のプロンプト。永続的な役割定義は
// システム指示に、出力フォーマットの強制は各プロバイダの構造化出力機能に分離する。
// これにより、モデルが素のテキストで JSON を返して整形が崩れる・前後に余計な
// 説明文が付く、といった失敗パターンを構造的に防いでいる。
// （このファイルの型付きスキーマはGemini用。Claude/GPT用のZodスキーマは
//   lib/analysis/schema.ts にあり、両者は必ず一致させること）
// ============================================================================
...
3. **和暦・旧字体・異体字の正規化**: 明治・大正・昭和・平成・令和の年号はすべて西暦（YYYY-MM-DD）に変換する。変換不能・判読不能・記載が「不詳」等の場合は、変換後フィールドを null とし、original_date / 原文表記に読み取れた文字をそのまま保持する。氏名の旧字体・異体字は、戸籍上の表記を name_original にそのまま残したうえで、現代の一般的な字体があれば name フィールドに用いてよい（字体を変えていない場合の name_original は null）。
4. **堅牢性**: フォーマット不能・判読不能な情報は original_ 系フィールドに原文（または「判読不能」）を保持し、変換後フィールドは null とする。情報の欠損よりも、誤った断定を避けることを優先する。
5. **「記載が無い」と「読めない」の区別**: 記載はあるが判読できなかった項目は、その人物の unreadable にキーを列挙する（name / sex / birth_date / death_date / birth_place / death_place / relation_to_family_head）。**記載自体が無い項目は unreadable に含めない。** 読めなかった箇所は画面上で「読み取りに失敗しました」と赤字で示され、担当者が原本にあたる起点になる。逆に、記載が無いものを unreadable に入れると、存在しない記載を探させることになる。
6. **完全性**: 戸籍にわずかでも言及がある人物は、関係性や生死が不明でも people に必ず含める。死亡・除籍・転籍で情報が乏しい人物も省略しない。
7. **推測の禁止**: 記載のない情報を文脈から推測して埋めない（sex 以外の項目も同様）。不明な値は null にする。
8. **ワンショット実行**: 修正が不要な、完成された構造化データを一度で出力することを目標とする。`

// ============================================================================
// タスク指示（ユーザーターンの本文。PDFファイルと共に送信する）
// ============================================================================
export const KOSEKI_TASK_PROMPT = `添付した戸籍謄本・除籍謄本を解析してください。

- **添付が複数ある場合、それらは同じ書類の各ページ・各面であり、ページ順に並んでいます。** 1通の戸籍として通しで読み、1つの people/families データにまとめてください。
...
- 記載はあるが判読できなかった項目は、その人物の unreadable にキーを列挙してください。記載自体が無い項目は含めないでください。
      ...
      items: {
        ...
        properties: {
          ...
          unreadable: {
            type: Type.ARRAY,
            description:
              '記載はあるが判読できなかった項目のキー。記載自体が無い項目は含めない。',
            items: {
              type: Type.STRING,
              format: 'enum',
              enum: [
