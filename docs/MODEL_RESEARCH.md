...
> **2026-09-24 追記：この表の Gemini 3.1 Pro は思考の分を含んでいない。**
> Gemini 3.x は既定で深く考え（high。3.1 Pro は切れない）、思考のトークンは出力として
> 課金される（3.1 Pro で $12/100万）。思考に2,000トークン使えば1ページ約¥3.6が上乗せになる。
> 以前は実測ログも思考分を数えていなかった（`candidatesTokenCount` だけを見ていた）。
> 現在は `thoughtsTokenCount` を出力に足して記録し、ログとベンチマークに内訳を出す。
>
> 1社で読む場合の安い候補（2026-09-24 時点・思考分を除く）:
>
> | モデル | 単価（入/出 per 1M） | 1ページ |
> |---|---|---:|
> | Gemini 3.1 Pro | $2 / $12 | ¥6.3 |
> | Gemini 3.1 Flash | $0.75 / $3.75（**2027-01-01 から $1.50 / $7.50**） | ¥2.1（2027年〜 ¥4.2） |
> | Gemini 3.1 Flash-Lite | $0.25 / $1.50 | ¥0.8 |
>
> Flash 系の手書き戸籍での精度は未測定。**切り替えは実データのベンチマークで決める。**
...
**2026-09-24 から既定では使わない（1社で読む）。**当初は「opt-inにすると一度も動かないまま
終わる」として既定で有効にしていたが、既定の組み合わせ（Gemini 3.1 Pro ＋ Claude Opus 5）では
読み取りの費用が1ページ約¥6→約¥28と4倍以上になる。照合は読み取りそのものの精度を上げる
ものではなく（ねらいは下のとおり確認箇所を絞ること）、論理矛盾の検出
（`utils/consistency.ts`）は照合なしでも動く。費用に見合う場面でだけ有効にする。

「決めたのに動かない」状態は避けたいので、照合先を指定しただけでも有効になるようにしてある。

```bash
ANALYSIS_ENSEMBLE=true                # 照合を使う
ANALYSIS_ENSEMBLE_PROVIDER=openai     # 任意。指定すると照合が有効になる（未指定ならキーのある別プロバイダ）
ANALYSIS_ENSEMBLE_MODEL=gpt-5.2       # 任意。安く照合するなら GPT-5.2（1ページ約¥3.4）
ANALYSIS_ENSEMBLE=false               # 上の2つより優先して止める
```
...
**原価:** 照合先の単価がそのまま上乗せになる。Claude Opus 5 なら1ページ約¥21、
GPT-5.2 なら約¥3.4。使う場合は安い照合先を選ぶ。

## 検討して採用しなかったもの：Jev（TypeSafe AI、2026-09-15 公開）

「型の決まった答えを返す」安価なモデル（入力 $0.042/100万トークン、出力無料）。
読み取り費用を下げられるかを検討したが、**現時点では使いどころがない**。

- **画像を読めない（テキストのみ）。**費用の大半は戸籍の画像を読む部分で、ここは置き換えられない
- **氏名・日付などの自由な値を返せない。**あらかじめ決めた選択肢・はい/いいえ・点数だけを返す。
  戸籍から取り出したいのはまさに自由な値
- 選択肢で答えられる判断（続柄の分類、同一人物かどうか、要確認かどうか）は、
  いまは読み取りと同じ1回の呼び出しの中か、無料の決定的な処理（`utils/consistency.ts`・
  `utils/mergePersons.ts`）で済んでいる。置き換えても安くならない
- 日付の厳密な比較は、提供元自身が不得意として挙げている（元号の矛盾検出はここに当たる）
- **データの扱いが確認できない。**学習への利用・保存・日本語対応が公開されておらず、
  早期アクセス（順番待ち）の段階。戸籍を送る先として `AI_DATA_POLICY.md` の条件を満たせない

判断を大量に繰り返す工程（例：大量の案件の振り分け）が将来できたら、データの扱いが
確認できた時点で再検討する。
