...
解析のたびに**別プロバイダのモデルでも解析し、結果を突き合わせて食い違いを洗い出す**。

**APIキーを2社以上設定すると既定で有効になる。** opt-inにすると設定を知らないまま
運用が始まり、実装したのに一度も動かないまま終わるため。1社分しかキーがない場合は
照合しようがないので静かに無効になる。
...
**解析は2つ同時に走る。** 逐次にすると待ち時間が単純に2倍になり、既定で有効にできない。
...
- **照合は先頭候補を前提に開始する。** primaryがフォールバックして照合先と同じ
  プロバイダになった場合は、比較しても意味がないため結果を破棄する
